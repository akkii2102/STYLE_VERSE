...
def product_detail(request, pk):
    ...
    # Determine model type string for this product
    prod_model_type = getattr(product, 'model_name', 'product')
    ...
    # ── Real DB Reviews for this specific product ────────────
    reviews_qs = ProductReview.objects.filter(
        model_type=prod_model_type,
        object_id=pk
    )
    ...
    if total > 0:
        ...
        avg_rating = round(agg['avg'] or 5, 1)
        rating_counts = {r: reviews_qs.filter(rating=r).count() for r in range(1, 6)}
        rating_stats = {
            'average':    avg_rating,
            'total':      total,
    ...
    specifications = [
        ...
        ('Guarantee',         '100% Quality & Authenticity Guaranteed'),
    ]
...
def submit_review(request, pk):
    """POST-only: save a product review and redirect back to the product detail page."""
    ...
    # Use logged-in user's name (the form field is disabled so not submitted)
    author_name = request.user.get_full_name() or request.user.username

    if not title or not comment:
        messages.error(request, 'Please fill in the review title and comment.')
        return redirect(f'/product/{pk}/?type={model_type}#reviews')
    ...
    ProductReview.objects.create(
        ...
        title       = title,
        comment     = comment,
        is_verified = True,   # logged-in users count as verified
    )

    messages.success(request, f'Thank you, {author_name}! Your review has been published.')
    return redirect(f'/product/{pk}/?type={model_type}#reviews')
